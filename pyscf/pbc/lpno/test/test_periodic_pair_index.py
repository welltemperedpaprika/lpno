...
Periodic pair storage uses the reference cell only (``i`` in the reference
cell, ``j`` over the full supercell).  Conjugate / inverse cells and the two
ordered orientations of an off-diagonal pair are physically distinct and must
*not* be averaged.  These tests pin that behavior for the ported
``kdOSVMP2._energy_corr_pair`` / ``KPNOMP2._sum_e_corr`` and the
``PeriodicPairIndex`` cell algebra.

Tests for PairMasks / PairEnergyBookkeeping are omitted; those classes are not
...
class _FakeKdosvPairDomain:
    def __init__(self, raw_ed_by_pair):
        self.K = _TupleIndexedPairData({
            pair: np.array([[ed]], dtype=float)
            for pair, ed in raw_ed_by_pair.items()
        })
        self.J = _TupleIndexedPairData({
            pair: np.array([[0.0]], dtype=float)
            for pair in raw_ed_by_pair
        })
