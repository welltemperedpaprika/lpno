...
def get_kdosv_eris(osv, eris, near_list, with_K=True, with_J=True, max_memory=None):
    ...
    _use_proj = (
        hasattr(eris, "_project_blk") and getattr(eris, "_ovL_k_R", None) is not None
    )
    ...
    for i, j in near_list:
        if _use_batched_dosv:
            ...
            iALR, iALI = eris._project_blk(i, osv.u[i])
        else:
        ...
        if j == i:
            ...
            if _use_batched_dosv:
                ...
                if with_K:
                    jBLR, jBLI = eris._project_blk(j, osv.u[j])
                    K[i, j] = np.empty((nosv[i], nosv[j]))
                    zdotCNtoR(iALR, iALI, jBLR.T, jBLI.T, cR=K[i, j])
                if with_J:
                    jALR, jALI = eris._project_blk(j, osv.u[i])
                    iBLR, iBLI = eris._project_blk(i, osv.u[j])
                    J[i, j] = np.empty((nosv[i], nosv[j]))
                    zdotCNtoR(jALR, jALI, iBLR.T, iBLI.T, cR=J[i, j])
            else:
...
def get_kdosv_eris_pao(
    osv, eris, pao, near_list, with_K=True, with_J=True, max_memory=None
):
    log = logger.new_logger(eris)
    if max_memory is None:
        max_memory = eris.max_memory
    ...
    for i, j in near_list:
        ivLR, ivLI = eris.get_occ_blk(i, i + 1)
        ivLR, ivLI = ivLR[0], ivLI[0]
        idx_i = pao.get_pao_idx(i)
        ivLR_i = ivLR[idx_i]
        ivLI_i = ivLI[idx_i]
        iALR = lib.dot(osv.u[i].T.conj(), ivLR_i)
        iALI = lib.dot(osv.u[i].T.conj(), ivLI_i)
        if i == j:
            Kii = np.empty((nosv[i], nosv[i]))
            zdotCNtoR(iALR, iALI, iALR.T, iALI.T, cR=Kii)
            ...
            idx_j = pao.get_pao_idx(j)
            jvLR_j = jvLR[idx_j]
            jvLI_j = jvLI[idx_j]
            jBLR = lib.dot(osv.u[j].T.conj(), jvLR_j)
            jBLI = lib.dot(osv.u[j].T.conj(), jvLI_j)
            ...
            if with_J:
                ...
                jvLR_i = jvLR[idx_i]
                jvLI_i = jvLI[idx_i]
                jALR = lib.dot(osv.u[i].T.conj(), jvLR_i)
                jALI = lib.dot(osv.u[i].T.conj(), jvLI_i)
                ivLR_j = ivLR[idx_j]
                ivLI_j = ivLI[idx_j]
                iBLR = lib.dot(osv.u[j].T.conj(), ivLR_j)
                iBLI = lib.dot(osv.u[j].T.conj(), ivLI_j)
                J[i, j] = np.empty((nosv[i], nosv[j]))
                zdotCNtoR(jALR, jALI, iBLR.T, iBLI.T, cR=J[i, j])
...
def get_kpsv_eri(
    ...
    for i in range(nlo_per_cell):
        if not _use_proj and not _use_cache:
            ivLR, ivLI = eris.get_occ_blk(i, i + 1)
            ivLR, ivLI = ivLR[0], ivLI[0]
        ...
        for j, fac, epsv, wpsv, ni, components in pair_data:
            if _use_cache:
                ...
                iALR, iALI = eris._project_blk(i, upsv)
                jBLR, jBLI = eris._project_blk(j, upsv)
            else:
                upsv = lib.dot(osv.u[i], wpsv[:ni]) + lib.dot(osv.u[j], wpsv[ni:])
...
def pbc_pair_dipole_pao(cell, lo_coeff, nlo_per_cell, moe_lo, pao, osv):
    ...
    frac_coords = np.dot(Rvecpair, inv_lat.T)
    frac_coords -= np.rint(frac_coords)
    Rvecpair = np.dot(frac_coords, lat.T)
    Rpair = np.linalg.norm(Rvecpair, axis=-1)
    diplosv = []
    for i in range(nlo_per_cell):
        d_half = einsum("xpq,p->xq", dipao, lo_coeff[:, i].conj())

        dipir = einsum("xq,qa->xa", d_half, pao.get_orbital_pao_coeff(i))
        diplosv.append(einsum("xa,aA->xA", dipir, osv.u[i]))
    epair = np.zeros_like(Rpair)

    for i in range(nlo_per_cell):
        i_ref, _ = map_global_lo_to_ref_R(i, nlo_per_cell, nlo)
        for j in range(nlo):
            dist = Rpair[i, j]
            if dist < 1e-3:
                continue

            j_ref, _ = map_global_lo_to_ref_R(j, nlo_per_cell, nlo)

            Rbar = Rvecpair[i, j] / dist
            Di = diplosv[i_ref]
            Dj = diplosv[j_ref]
            vab = einsum("xA,xB->AB", Di, Dj)
            proj_i = np.dot(Rbar, Di)  # (N_OSV_i,)
            proj_j = np.dot(Rbar, Dj)  # (N_OSV_j,)
            vab -= einsum("A,B->AB", proj_i, proj_j) * 3
            denom = moe_lo[i] + moe_lo[j] - (osv.e[i][:, None] + osv.e[j])
            tab = vab.conj() / denom

            eij = einsum("ab,ab->", tab, vab) / (dist**6) * 4.0
...
def pbc_pair_ewald_dipole_pao(cell, lo_coeff, nlo_per_cell, moe_lo, pao, osv):
    """Ewald-summed dipole approximation for PAO-basis periodic pair energies."""
    nlo = lo_coeff.shape[1]
    ...
    for i in range(nlo_per_cell):
        ...
        for j in range(nlo):
            ...
            j_ref, _ = map_global_lo_to_ref_R(j, nlo_per_cell, nlo)
            T = _ewald_dipole_tensor(Rvecpair[i, j], eta, Ls, Gv, vol)
            Di = diplosv[i_ref]
            Dj = diplosv[j_ref]
            vab = einsum("xA,xy,yB->AB", Di, T, Dj)
            denom = moe_lo[i] + moe_lo[j] - (osv.e[i][:, None] + osv.e[j])
            tab = vab.conj() / denom
            epair[i, j] = einsum("ab,ab->", tab, vab) * 4.0
...
class PairDomain_kdOSV_PAO(PairDomain_kdOSV):
    def __init__(
        self, eris, osv, pao, pair_mask, nlo_per_cell, nlo, near_list, with_J=True
    ):
        self.eris = eris
        self.osv = osv
        self.pair_mask = pair_mask
        ...
        self.J = None
        self.pao = pao
        self.build(pao, with_J)

    def build(self, pao, with_J):
        self.K, self.J = get_kdosv_eris_pao(
            self.osv, self.eris, pao, self.near_list, with_J=with_J
        )

    @property
    def S(self):
        return self.osv.get_ovlp(self.pao)
