...
def make_translation_symmetric_lo(cell, kmf, kpts, kmesh):
    """Build translation-symmetric supercell occupied LOs from Bloch orbitals."""
    from pyscf.pbc.lib.kpts import KPoints
    if isinstance(kpts, KPoints):
        if len(kpts) == kpts.nkpts:
            order = np.asarray(kpts.bz2ibz, dtype=int)
            cocc_k = [kmf.mo_coeff[x][:, kmf.mo_occ[x] > 0] for x in order]
            kpts_arr = np.asarray(kpts.kpts)
        else:
            try:
                from pyscf.pbc.lo.base import remove_trs_mo
                cocc_k = np.asarray([kmf.mo_coeff[k][:, kmf.mo_occ[k] > 0]
                                     for k in range(len(kmf.mo_coeff))])
                cocc_k = list(remove_trs_mo(cocc_k, kpts))
            except Exception:
                nkpts_ibz = kpts.nkpts_ibz
                cocc_k = [None] * kpts.nkpts
                for ki in range(nkpts_ibz):
                    ids = np.where(kpts.bz2ibz == ki)[0]
                    c = kmf.mo_coeff[ki][:, kmf.mo_occ[ki] > 0]
                    if ids.size == 1:
                        cocc_k[ids[0]] = c
                    elif ids.size == 2:
                        cocc_k[ids[0]] = c.conj()
        ...
        cocc_k = [kmf.mo_coeff[k][:, kmf.mo_occ[k] > 0]
                  for k in range(len(kpts_arr))]

    nkpts = len(kpts_arr)
    nao = cell.nao_nr()
    nocc_cell = cell.nelectron // 2
    cocc_k = np.asarray(cocc_k)
    _, phase = k2gamma.get_phase(cell, kpts_arr, kmesh)
    ncell = phase.shape[0]
    ...
    for ik, k in enumerate(kpts_arr):
...
def _realize_selfpaired(cell, kpt, c):
    """Return a real S-orthonormal basis of span(c) at a TR-invariant k-point."""
    if np.linalg.norm(c.imag) < 1e-10:
        return c
    s = cell.pbc_intor('int1e_ovlp', kpts=np.reshape(kpt, (1, 3)))[0]
    s = s.real
    nvec = c.shape[1]
    m = np.hstack([c.real, c.imag])
    w = m.T @ s @ m
    e, v = np.linalg.eigh(w)
    idx = np.argsort(e)[::-1][:nvec]
    if e[idx[-1]] < 1e-10 * e[idx[0]]:
        raise RuntimeError(f'TRS realization failed: rank-deficient span at k-point {kpt}')
    q = (m @ v[:, idx]) / np.sqrt(e[idx])
    fid = np.linalg.norm(q.T @ (s @ c)) ** 2 / nvec
    if abs(fid - 1.0) > 1e-8:
        raise RuntimeError(f'TRS realization failed: span fidelity {fid:.12f} != 1 at k-point {kpt}')
    return q.astype(c.dtype)


def enforce_trs_gauge(cell, kpts, mo):
    """Rotate per-k MO blocks to the time-reversal gauge C(-k) = C(k)*."""
    kpts = np.asarray(kpts)
    mo = np.array(mo, copy=True)
    frac = cell.get_scaled_kpts(kpts)

    def key(f):
        return tuple(np.round(np.round(f % 1.0, 8) % 1.0, 6))

    index = {key(f): i for i, f in enumerate(frac)}
    fixed = np.zeros(len(kpts), bool)
    for i in range(len(kpts)):
        if fixed[i]:
            continue
        j = index.get(key(-frac[i]))
        if j is None:
            raise RuntimeError(f'TRS partner of k-point {i} not in mesh')
        if j == i:
            mo[i] = _realize_selfpaired(cell, kpts[i], mo[i])
        else:
            mo[j] = mo[i].conj()
            fixed[j] = True
        fixed[i] = True
...
def make_lo_kpipek(cell, kmf, kpts, kmesh, frozen_per_kpt=None, verbose=0):
    """Generate localized occupied orbitals in the supercell using periodic Pipek-Mezey (kpipek).

    Args:
        cell : pbc.gto.Cell
        kmf : pbc.scf.KRHF
        kpts : KPoints or ndarray of shape (nkpts, 3)
        kmesh : list or tuple of 3 ints [nx, ny, nz]
        frozen_per_kpt : int, optional
            Number of core orbitals frozen per k-point. If None, auto-detected
            via guess_frozen(cell).
        verbose : int, optional
            Verbosity level for localization. Default 0.

    Returns:
        lo_coeff : ndarray of shape (nao_supercell, nlo)
            Localized occupied orbital coefficients in the supercell AO basis.
        kfrozen : int
            Total number of frozen core orbitals across the supercell.
    """
    from pyscf.pbc.lib.kpts import KPoints
    from pyscf.pbc.lo import kpipek
    from pyscf.lpno.tools import guess_frozen

    if isinstance(kpts, KPoints):
        nkpts = kpts.nkpts
        kpts_bz = kpts.kpts
    else:
        kpts_bz = np.asarray(kpts)
        nkpts = len(kpts_bz)
    ...
    mo_val = np.asarray([kmf.mo_coeff[k][:, frozen_per_kpt:nocc]
                         for k in range(len(kmf.mo_coeff))])

    # If full-BZ input without TRS, ensure C(-k) = C(k)* gauge for KPMReal
    if not isinstance(kpts, KPoints) or len(kpts) == kpts.nkpts:
        mo_val = enforce_trs_gauge(cell, kpts_bz, mo_val)

    mlo = kpipek.KPMReal(cell, mo_val, kpts)
    mlo.verbose = verbose
    mlo.kernel()
    while True:
    ...
    im_norm = float(np.linalg.norm(orbloc.imag))
    if im_norm > 1e-4:
        raise RuntimeError(f"Gauge-broken localization: Wannier imag norm {im_norm:.3e} > 1e-4")

    return orbloc.real, kfrozen
